...
const
  ...
  LibPropertyStr: AnsiString = '<?xml version="1.0" standalone="yes"?>' + '<LIB_DESCR>' +
    '<INFO TYPE="RS" DESCR="Port RS (protocol Modbus+)" SIGN="MCOM"/>' + '<PARAMS>' +
    ...
    '<PARAM DESCR="RTU/ASCII" TYPE="SELECT" DEFAULT="19200"/>' + '</PARAMS>' + '</LIB_DESCR>';
...
{$INCLUDE ..\Common\CmmBaseDef.inc}
{$INCLUDE ..\Common\FileAccessDef.inc}
{$INCLUDE ..\Common\StdModbusDef.inc}
{$INCLUDE ..\Common\MemoryAccessDef.inc}
{$INCLUDE ..\Common\TerminalDef.inc}

function GetComAccessHandle(ComNr: integer; var ComHandle: THandle; var SemHandle: THandle): boolean;
procedure CloseComAccess(AccId: TAccId);
...
begin
  ...
  if GetComAccessHandle(ComNr, ComHandle, SemHandle) then
  begin
    if GetComAcces then
    begin
      Result := SetupState;
...
begin
  Result := PAnsiChar(LibPropertyStrV2);
end;
...
begin
  ...
  begin
    if (ComList.Items[i] as TComItem).ComNr = ComNr then
    begin
      it := ComList.Items[i] as TComItem;
      break;
    end;
  end;
  ...
  begin
    ...
    it.ComHandle := CreateFile(pchar(s), GENERIC_READ or GENERIC_WRITE, 0, nil, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, 0);
