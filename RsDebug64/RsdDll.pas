...
type
  ...
  public
    ...
    function  GetErrStr(Code : TStatus;S : pChar; Max: integer): boolean; overload;
...
type
  ...
  TAddDev   = function (ConnectStr : pChar): TAccId; stdcall;
...
// funkcje podstawowe Modbusa
  ...
  TGetErrStr = function (Id :TAccId; Code :TStatus; S : pChar; Max: integer): boolean;  stdcall;
  TRegisterCallBackFun = function(Id :TAccId; CmmId : integer; CallBackFunc : TCallBackFunc): TStatus; stdcall;
  TGetDrvParamList = function(ToSet : boolean): pchar; stdcall;
  TGetDrvStatus = function(Id :TAccId; ParamName : pchar; ParamValue :pchar; MaxRpl:integer): TStatus; stdcall;
  TSetDrvParam = function(Id :TAccId; ParamName : pchar; ParamValue :pchar): TStatus; stdcall;
...
function  TCmmDevice.GetErrStr(Code : TStatus;S : pChar; Max: integer): boolean;
var
...
begin
  ...
  begin
    ...
    if GetErrStr(Code,pchar(Result),length(Result)-1) then
...
begin
  ...
  if Assigned(_SetDrvParam) then
    Result := _SetDrvParam(FID,pchar(ParamName),pchar(ParamValue))
  else
    Result := stNoImpl;
end;
...
begin
  ...
  begin
    ...
    Result := _GetDrvStatus(FID,pchar(ParamName),pchar(ParamValue),Length(ParamValue)-1);
