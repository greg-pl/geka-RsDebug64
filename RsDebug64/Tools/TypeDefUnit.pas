...
type
  ...
  TSysType = (ssCHAR, ssINT, ssLONG, ssINT64, ssFLOAT, ssSFLOAT, ssDOUBLE, ssSDOUBLE);

const
  ShowModeTxt: array [TShowMode] of string = ('DEFAULT', 'SIGN', 'USIGN', 'HEX', 'CHAR', 'FRAC');
  ShowModeSymb: array [TShowMode] of char = ('?', 'S', 'U', 'H', 'C', 'R');

  SysVarTxt: array [TSysType] of string = ('int8', 'int16', 'int32', 'int64', 'float', 'sfloat', 'double', 'sdouble');
  SysVarSize: array [TSysType] of integer = (1, 2, 4, 8, 4, 4, 8, 8);
...
type
  ...
  public
    ...
    function LoadFromText(ByteOrder: TByteOrder; var Dt: pbyte; TypeList: THTypeList; TxMode: TShowMode;
      var Tx: string): boolean;
    ...
    case T of
      ssCHAR:
        begin
          case TxMode of
            smSIGN:
              Result := Result + InttoStr(ShortInt(Dt^));
            smUNSIGN:
              Result := Result + InttoStr(Dt^);
            smHEX:
              Result := Result + '0x' + IntTohex(Dt^, 2);
            smCHAR:
              if (Dt^ >= $20) then
                ...
                Result := Result + '.';
            smFRAC:
              Result := Result + FloatToFracStr(ShortInt(Dt^) / $80);
          end;
          inc(Dt);
        end;
      ssINT:
        begin
          ...
          case TxMode of
            smSIGN:
              Result := Result + InttoStr(SmallInt(w));
            smUNSIGN:
              Result := Result + InttoStr(w);
            smHEX:
              Result := Result + '0x' + IntTohex(w, 4);
            smFRAC:
              Result := Result + FloatToFracStr(SmallInt(w) / $8000);
          end;
        end;
      ssLONG:
        begin
          ...
          case TxMode of
            smSIGN:
              Result := Result + InttoStr(longint(D));
            smUNSIGN:
              Result := Result + InttoStr(int64(D));
            smHEX:
              Result := Result + '0x' + IntTohex(D, 8);
            smFRAC:
              Result := Result + FloatToFracStr(longint(D) / $80000000);
          end;
        end;
      ssFLOAT:
        begin
      ...
      ssINT64:
        begin
          ...
          case TxMode of
            smSIGN, smUNSIGN:
              Result := Result + InttoStr(I64);
            smHEX:
              Result := Result + '0x' + IntTohex(I64, 16);
            smFRAC:
              Result := Result + FloatToFracStr(I64 / $8000000000000000);
          end;
        end;
      ssDOUBLE:
        begin
...
function THType.LoadFromText(ByteOrder: TByteOrder; var Dt: pbyte; TypeList: THTypeList; TxMode: TShowMode;
  var Tx: string): boolean;
var
  T: TSysType;
  Tck: string;
        ...
        ssCHAR:
          begin
            ...
            begin
              Tck := GetTocken(Tx);
              case TxMode of
                smSIGN, smUNSIGN, smHEX:
                  Dt^ := byte(StrToIntChex(Tck));
                smFRAC:
                  Dt^ := round(StrToFloat(Tck) * $80);
              end;
            end;
            inc(Dt);
          end;
...
begin
  ...
  try
    ...
    if SL.Count >= 3 then
      ...
      Rep := 1;
    if SL.Count >= 4 then
      ShowMode := StrToShowMode(SL.Strings[3], smSIGN)
    else
      ShowMode := smSIGN;
  finally
    SL.Free;
